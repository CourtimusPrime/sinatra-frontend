...
function FinalizeAccount({ onboardData, setCanProceed }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-gray-700">Here's a final look before we create your account:</p>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">{onboardData.featured_playlists.length} featured playlists</p>
        </div>
      </div>
      ...
      <div>
        ...
        <ul className="list-disc ml-6 text-sm">
          {onboardData.selected_playlists.map((p) => (
            <li key={p.id}>{p.name} ({p.tracks} songs)</li>
          ))}
        </ul>
      </div>
